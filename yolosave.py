...
def webcam_stream(save_img = True):
    ...
    while True:
        if not available_port(SERIAL_PATH):
            ...
            print("on devrait appeler llava")
            if found:
                try:
                    ...
                    # if available_port(SERIAL_PATH):
                    print("on appelle vraiment serge llama")
                    subprocess.Popen([str(python), str(llava)], shell=False)
                        
                    sleep(5)
                    # print("debut du sleep")
                    # print("fin du sleep")
                    # else:
                    # print("en fait on n'appelle pas llama car le port est pas dispo oupsi")
